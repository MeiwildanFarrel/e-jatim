...
### F3. Alternative Credit Scoring (ACS) — ✅ Growth/Stability/Risk dihitung dari ledger real (13 Juli), Reputation masih placeholder, benchmarking model ML diterima & diarsipkan (18 Juli)
- `Score = w₁·Growth + w₂·Stability + w₃·Reputation − w₄·RiskFactor`.
- ⚠️ **KEPUTUSAN TIM (12-13 Juli) — model ML Anggota B TIDAK dipakai untuk skor runtime Bu Sari.** Model itu dilatih di dataset publik (Kaggle Home Credit) dengan **190 fitur profil individu** (pendidikan, pekerjaan, status pernikahan, kepemilikan aset, dst — lihat `HomeCredit_columns_description.csv`) yang tidak ada padanannya sama sekali di data ledger transaksi UMKM kita — jadi tidak bisa diterapkan langsung ke data Bu Sari. Random Forest + XGBoost (evaluasi AUC-ROC/F1/Gini/SHAP) tetap jadi **bukti metodologi di paper** (menunjukkan due diligence benchmarking), **bukan** komponen yang menghasilkan angka di runtime PoC ini.
- **📦 Paket benchmarking lengkap diterima dari Anggota B (18 Juli) & diarsipkan di `docs/acs-model-benchmarking/`** (di luar `src/`/`app/` — sengaja bukan bagian runtime): `best_model.pkl`, `scaler.pkl`, `feature_names.json`, `preprocessing_meta.json`, `benchmarking_results.csv`, `HomeCredit_columns_description.csv`, `acs_scoring_function.py`, dan 6 grafik di `charts/` (distribusi fitur turunan, benchmarking model, confusion matrix, SHAP importance/summary/breakdown komponen ACS). **Belum dipanggil di pipeline runtime** (konsisten dengan keputusan di atas) — murni bukti metodologi/lampiran paper.
- **Hasil benchmarking 3 model** (test set, dari `preprocessing_meta.json` + `benchmarking_results.csv`):

  | Model | AUC | F1 | Gini |
  |---|---|---|---|
  | **XGBoost (terbaik)** | **0,7436** | **0,8827** | **0,4871** |
  | Logistic Regression (baseline) | 0,737 | 0,760 | 0,474 |
  | Random Forest | 0,710 | 0,857 | 0,420 |

  XGBoost dipilih sebagai `best_model.pkl` (190 fitur setelah preprocessing, `random_seed=42`, SMOTE diterapkan untuk imbalance kelas).
- ⚠️ **Limitasi jujur untuk paper — recall sangat rendah pada threshold default**: confusion matrix `best_model.pkl` di test set (`charts/section5_confusion_matrix.png`) menunjukkan TN=42.353, FP=50, **FN=3.674, TP=50** — dari **3.724 kasus gagal bayar aktual** (TP+FN), model hanya benar menangkap **50 (Recall=1,3%)** pada threshold klasifikasi default 0,5. AUC (0,74) dan Gini (0,49) tetap wajar karena keduanya mengukur *ranking* probabilitas di semua threshold, bukan akurasi keputusan biner di satu titik potong — jadi tidak kontradiktif, tapi tetap harus ditulis eksplisit sebagai limitasi metodologi, bukan disembunyikan di balik angka AUC yang terlihat bagus. **Ini TIDAK memengaruhi `acs_scoring_function.py`**: fungsi itu memakai `model.predict_proba(...)` (probabilitas kontinu 0-1, `risk_score = prob_repay * 100`), bukan hasil klasifikasi biner threshold-0,5 — jadi limitasi recall di atas relevan untuk *evaluasi model sebagai classifier*, bukan untuk cara `risk_score` dihitung dalam skema ACS.
- ⚠️ **DUA versi bobot tentatif berbeda saat ini, BELUM direkonsiliasi**:
  - **Live di `src/lib/scoring/acsCalculator.ts`** (dipakai runtime dashboard Bu Sari): `w1=0,35 (Growth), w2=0,25 (Stability), w3=0,20 (Reputation), w4=0,20 (Risk)`.
  - **Usulan `preprocessing_meta.json` Anggota B** (`acs_weights_tentative`, juga dipakai default di `acs_scoring_function.py`): `w1=0,25, w2=0,25, w3=0,20, w4=0,30`.
  - Keduanya sama-sama ditandai "TENTATIF" oleh pemiliknya masing-masing. **Belum ada keputusan final mana yang dipakai** — menunggu validasi Pak Sena. **`acsCalculator.ts` TIDAK diubah** sesi ini; kalau nanti diputuskan pakai versi Anggota B, itu perubahan kode terpisah, bukan otomatis dari update dokumentasi ini.
...
- Bobot yang dipakai `combineAcsScore()` saat ini: `w1=0,35, w2=0,25, w3=0,20, w4=0,20` — **TENTATIF**, lihat perbandingan dengan usulan Anggota B di atas (dua bobot berbeda, belum direkonsiliasi).
